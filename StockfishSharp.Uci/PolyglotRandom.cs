// Tabella standard di costanti Zobrist del formato Polyglot (781 valori: 768 pezzo*casa, 4
// arrocco, 8 en passant, 1 tratto) — dati di interoperabilità universali del formato .bin
// (pubblicati da Fabien Letouzey, autore di Polyglot), copiati identici da ACMyChess.Engine
// (PolyglotRandom.cs), che a sua volta li usava già per lo stesso scopo. NON sono le costanti
// Zobrist interne di Stockfish (Zobrist.cs in StockfishSharp.Engine, generate da un PRNG diverso
// per la TT/repetition) — il formato .bin Polyglot richiede specificamente QUESTE, altrimenti le
// chiavi calcolate non troverebbero mai le voci del libro.
//
// Flow D1 del piano (docs/porting-master-plan.md) — NON un porting di Stockfish (che non ha un
// libro di aperture integrato), riuso della logica già funzionante in ACMyChess.

using StockfishSharp.Engine;

namespace StockfishSharp.Uci;

internal static class PolyglotRandom
{
    ...
    /// <summary>Indice 0..11 del "tipo di pezzo colorato" nello schema Polyglot: nero pari,
    /// bianco dispari (pedone nero 0, pedone bianco 1, ...).</summary>
    private static int PieceIndex(Piece pc)
    {
        int t = Types.TypeOf(pc) switch
        {
            PieceType.Pawn => 0, PieceType.Knight => 1, PieceType.Bishop => 2,
            PieceType.Rook => 3, PieceType.Queen => 4, PieceType.King => 5,
            _ => 0,
        };
        return (2 * t) + (Types.ColorOf(pc) == Color.White ? 1 : 0);
    }

    /// <summary>Chiave Zobrist Polyglot completa (pezzi+arrocco+en-passant+turno). Calcolata da
    /// zero ogni volta: <c>Position.Key</c> usa lo schema Zobrist interno di Stockfish (Zobrist.cs,
    /// generato da un PRNG diverso), incompatibile col formato .bin — a differenza di ACMyChess,
    /// che tiene la propria chiave Polyglot incrementale su Board, qui non serve: il libro si
    /// consulta solo nelle primissime mosse di una partita, il costo di un ricalcolo completo a
    /// ogni "go" è trascurabile. Indicizzazione delle case diretta (nessun flip di riga come in
    /// ACMyChess): Square.A1=0..H8=63 in StockfishSharp è già la stessa convenzione "rank dal
    /// basso, file da a" richiesta dal formato Polyglot.</summary>
    public static ulong ComputeKey(Position pos)
    {
        ulong key = 0;
        for (var s = Square.A1; s <= Square.H8; s++)
        {
            ...
            key ^= Values[(64 * PieceIndex(pc)) + (byte)s];
        }

        if (pos.CanCastle(CastlingRights.WhiteOo)) key ^= Values[768];
        if (pos.CanCastle(CastlingRights.WhiteOoo)) key ^= Values[769];
        if (pos.CanCastle(CastlingRights.BlackOo)) key ^= Values[770];
        if (pos.CanCastle(CastlingRights.BlackOoo)) key ^= Values[771];

        if (pos.EpSquare != Square.None && EnPassantCapturable(pos))
            key ^= Values[772 + (byte)Types.FileOf(pos.EpSquare)];

        if (pos.SideToMove == Color.White) key ^= Values[780];

        return key;
    }

    private static bool EnPassantCapturable(Position pos)
    {
        Color us = pos.SideToMove;
        ulong attackers = Attacks.PawnAttacksBb(pos.EpSquare, Types.Opposite(us)) & pos.Pieces(us, PieceType.Pawn);
        return attackers != 0;
    }
}
