...
| `movepick.h` + `movepick.cpp` + `history.h` | `StockfishSharp.Engine/MovePick.cs` (history) + `StockfishSharp.Engine/MovePicker.cs` (generazione a stadi) | ⚠️ **in corso, Flow A2 di `porting-master-plan.md`** — main/capture/continuation (6 livelli)/low-ply/TT-move/pawn (parziale) history fedeli; **generazione a stadi vera FATTA** (`MovePicker`, `Stages`, `Score<Type>`, `partial_insertion_sort` scalare, `select<Pred>`), killer move eliminate (la fonte le aveva già tolte). NON portati: i due usi mancanti di PawnHistory, `reduction()`/`statScore` che non sfruttano ancora tutta l'informazione di `MovePicker` |
| `search.h` + `search.cpp` | `StockfishSharp.Engine/Search.cs` | ⚠️ **in corso, Flow A1 di `porting-master-plan.md`** — negamax+PVS, quiescenza, TT (con `value_to_tt`/`value_from_tt` fedeli), mate distance pruning, null-move, RFP, aspiration windows, Razoring (Step 8), Futility pruning per mossa figlia (Step 9), cutNode tracciato in ricorsione, Internal Iterative Reduction (Step 11), ProbCut (Step 12+13), CorrectionHistory, vera formula di riduzione LMR (reduction()), Step 15 (potatura a profondità bassa), **Singular Extensions (Step 16, con multi-cut ed estensione negativa), bonus "countermove" su fail-low puro (Step 23), hindsight depth adjustment da priorReduction** — **Flow A1 COMPLETO** per uso single-thread. Un caveat di instabilità a profondità basse su una posizione, notato con Step 15, è ora molto migliorato (stabile 7-12, residuo solo "a freddo"/depth 6, vedi Search.cs). NON portato: Lazy SMP (Flow C) |
| `timeman.h` + `timeman.cpp` | `StockfishSharp.Engine/TimeManagement.cs` | ✅ fatto fedelmente (Flow A3 di `porting-master-plan.md`) — modello optimum/maximum, entrambe le modalità, move overhead, ponder. NON portato: nodestime, estensione dinamica del budget durante la ricerca |
