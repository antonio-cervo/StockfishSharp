...
| `movepick.h` + `movepick.cpp` + `history.h` | `StockfishSharp.Engine/MovePick.cs` | ⚠️ **in corso, Flow A2 di `porting-master-plan.md`** — main history, capture history (`CapturePieceToHistory`) fedeli, **continuation history parziale (solo ss-1, una tabella)**; killer move restano euristica NOSTRA (la fonte le ha eliminate). NON portati: generazione a stadi, continuation history ss-2..ss-6, pawn/low-ply/TT-move history |
| `search.h` + `search.cpp` | `StockfishSharp.Engine/Search.cs` | ⚠️ **in corso, Flow A1 di `porting-master-plan.md`** — negamax+PVS, quiescenza, TT (con `value_to_tt`/`value_from_tt` fedeli), mate distance pruning, null-move, RFP, LMR base, aspiration windows, Razoring (Step 8), Futility pruning per mossa figlia (Step 9), cutNode tracciato in ricorsione, Internal Iterative Reduction (Step 11), **ProbCut (Step 12+13)**. NON portati: Singular Extensions, multi-cut, correction history, la vera formula di riduzione LMR, Lazy SMP |
