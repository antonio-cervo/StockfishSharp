...
| `movepick.h` + `movepick.cpp` + `history.h` | `StockfishSharp.Engine/MovePick.cs` | ⚠️ **in corso, Flow A2 di `porting-master-plan.md`** — main history (`ButterflyHistory`) fedele, **continuation history parziale (solo ss-1, una tabella)**; killer move restano euristica NOSTRA (la fonte le ha eliminate). NON portati: generazione a stadi, capture history, continuation history ss-2..ss-6, pawn/low-ply/TT-move history |
