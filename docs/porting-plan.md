...
| `movepick.h` + `movepick.cpp` + `history.h` | `StockfishSharp.Engine/MovePick.cs` | ⚠️ **in corso, Flow A2 di `porting-master-plan.md`** — main history (`ButterflyHistory`) ora fedele (formula "a gravità" + bonus/malus di `update_all_stats`); killer move restano euristica NOSTRA (la fonte le ha eliminate). NON portati: generazione a stadi, capture/continuation/pawn/low-ply/TT-move history |
