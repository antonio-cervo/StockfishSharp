...
| `search.h` + `search.cpp` | `StockfishSharp.Engine/Search.cs` | ⚠️ **in corso, Flow A1 di `porting-master-plan.md`** — negamax+PVS, quiescenza, TT (con `value_to_tt`/`value_from_tt` fedeli), mate distance pruning, null-move, RFP, aspiration windows, Razoring (Step 8), Futility pruning per mossa figlia (Step 9), cutNode tracciato in ricorsione, Internal Iterative Reduction (Step 11), ProbCut (Step 12+13), CorrectionHistory, **vera formula di riduzione LMR (reduction()), Step 15 (potatura a profondità bassa) — quest'ultimo con un caveat aperto di instabilità a profondità basse su una posizione, vedi il commento in Search.cs**. NON portati: Singular Extensions (priorità alta, legata al caveat), multi-cut, Lazy SMP |
