...
| `movepick.h` + `movepick.cpp` + `history.h` | `StockfishSharp.Engine/MovePick.cs` | ⚠️ **in corso, Flow A2 di `porting-master-plan.md`** — main/capture/continuation (6 livelli)/low-ply/TT-move/pawn (parziale) history fedeli; killer move restano euristica NOSTRA (la fonte le ha eliminate). NON portata: generazione a stadi |
